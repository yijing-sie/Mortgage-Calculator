...
public class Mortgage {
 ...
 //define three constants that hold
 //the preferences key names for amount, years, and
 //rate.
  ...
  //include context to pass it to the getDefaultSharedPreferences
  public void setPreferences( Context context ) {
    SharedPreferences pref =
        PreferenceManager.getDefaultSharedPreferences( context );
    //we need edit() method to get a SharedPreferences.Editor reference
    //then we can pass it to SharedPreferences.Editor class
    SharedPreferences.Editor editor = pref.edit( );
    editor.putFloat( PREFERENCE_AMOUNT, amount );
    editor.putInt( PREFERENCE_YEARS, years );
    editor.putFloat( PREFERENCE_RATE, rate );
    //IMPORTATNT : need to commit() to acturally store it
    editor.commit( );
  }
}